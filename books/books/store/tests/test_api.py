...
class BooksAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.book_1 = Book.objects.create(name='Test_book 1', price=25, author_name='Author 1', owner=self.user)
    ...
    def test_get_filter(self):
        ...
        url = reverse('book-list')
        books = Book.objects.filter(id__in=[self.book_2.id, self.book_3.id]).annotate(  # annotate likes
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
        ).order_by('id')
    ...
    def test_get_search(self):
        ...
        url = reverse('book-list')
        books = Book.objects.filter(id__in=[self.book_3.id, self.book_1.id]).annotate(  # annotate likes
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
        ).order_by('id')
    ...
    def test_update_not_owner(self):###  не владелец книги
...
class BooksRelationTestCase(APITestCase):
    ...
    def test_rate_wrong(self):              ####тест падает - райтинг которого нет
        url = reverse('userbookrelation-detail', args=(self.book_1.id,))
        data = {
            "rate": 6,
        }
